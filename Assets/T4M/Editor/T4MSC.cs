...
public partial  class T4MSC : EditorWindow {
	...
	void OnGUI () 
	{
		...
		GUILayout.Label("T4M SC 2018修正版");
	...
	void InitPincil()
	{
		...
		if (CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.HasProperty("_Splat5")){
			...
			TexTexture[0]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat0");
			TexTexture[1]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat1");
			TexTexture[2]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat2");
			TexTexture[3]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat3");
			TexTexture[4]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat4");
			TexTexture[5]=CurrentSelect.gameObject.GetComponent <T4MObjSC>().T4MMaterial.GetTexture("_Splat5");
	...
	{	
		if (vertexInfo ==0 && trisInfo ==0 && partofT4MObj ==0){
			...
			else
			{
                ...
                if (CurrentSelect.GetComponent<T4MObjSC>())
                {
                    T4MTool.ConvToOneGUI(CurrentSelect);
	 ...
	 void ConvertUTerrain()
	{
        ...
        string path2 = T4MPrefabFolder + "Terrains/Texture/" + FinalExpName + "_ctrl2.png";
        bool hasPath2 = false;
...

        {
            ...
            if (null != TextureI)
            {
                TextureImporterPlatformSettings settingAndroid = TextureI.GetPlatformTextureSettings("Standalone");
                settingAndroid.overridden = true;
                settingAndroid.format = TextureImporterFormat.RGBA32;
                TextureI.SetPlatformTextureSettings(settingAndroid);
                TextureI.isReadable = true;
                TextureI.anisoLevel = 9;
                TextureI.mipmapEnabled = false;
                TextureI.wrapMode = TextureWrapMode.Clamp;
            ...
            {
                Debug.LogError("tex "+path+" not found");
            }
        ...
        {
            //Modification de la Texture 
            TextureImporter TextureI = AssetImporter.GetAtPath(path2) as TextureImporter;
            if (null != TextureI)
            {
                TextureImporterPlatformSettings settingAndroid = TextureI.GetPlatformTextureSettings("Standalone");
                settingAndroid.overridden = true;
            ...
            {
                Debug.LogError("tex " + path2 + " not found");
            }
        ...
        if (hasPath2)
        {
            Tmaterial = new Material(Shader.Find("T4MShaders/ShaderModel3/Diffuse/T4M 6 Textures"));
        }
        else
        {
        ...
        {
            {
                Texture test = (Texture)AssetDatabase.LoadAssetAtPath(path2, typeof(Texture));
            ...
            }
            {
                ...
                Tmaterial.SetTexture("_Control2", test);
            }
    ...
    public static  void ResetMaskText()
    {
